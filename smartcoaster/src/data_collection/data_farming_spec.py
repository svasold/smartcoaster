...
levels = [0]
drinks = ['water', 'beer', 'red wine', 'white wine', 'apple juice', 'cola', 'fanta', 'mineral water', 'wheat beer', 'orange juice', 'empty glass', 'tea', 'coffee', 'milk coffee']
drink_index = 0
drink = 'water'
            ...
            with open(CSV_FILE, 'a') as csvfile:
                ...
                for level in levels:
                    ...
                    for brightness_index, spec_led_brightness in enumerate([1, 3]):
                        lcdUpdate("level: " + str(level) + " led: " + str(spec_led_brightness))
                        ...
                        for i in range(ITERATIONS):
                            ...
                            if(container != "mug"):
                                ...
                                spec_data1[brightness_index].append(spec1.get_calibrated_values())
                                spec1.disable_main_led()
                            else:
                                spec_data1[brightness_index].append([0, 0, 0, 0, 0, 0])

                        #from the top
                        spec4.enable_main_led()
                        spec_data4[brightness_index].append(spec4.get_calibrated_values())
                    ...
                    for sample_index in range(ITERATIONS):
                        ...
                        dataset.extend(spec_data1[0][sample_index])
                        dataset.extend(spec_data4[0][sample_index])
                        dataset.extend(spec_data1[1][sample_index])
                        dataset.extend(spec_data4[1][sample_index])
